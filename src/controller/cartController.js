...
const updateCart =async (req,res)=>{
    ...
    if(Object.keys(a).length!=0) return res.status(400).send({status:false, message:`please remove ${object.keys(others)} key`});
    ...
    if(removeProduct==0){
       ...
       return res.status(201).send({status:true, message:"success", data:upadatedProducts})
    }
    if(removeProduct==1){
        for(let i=0; i<cartExist.items.length;i++){
        if(cartExist.items[i].productId==productId){
          if(cartExist.items[i].quantity>1){
            ...
            return res.status(201).send({status:true, message:"success", data:upadatedProduct})
          } cartExist.totalPrice-=productExist.price*cartExist.items[i].quantity
            cartExist.totalItems-=1
             cartExist.items.splice(i,1)
         let upadatedProducts= await cartModel.findOneAndUpdate({_id:cartId},cartExist,{new:true})
...
module.exports={updateCart,createCart,getCart}